...
export default function MyOrder() {
  ...
  const fetchMyOrder = async () => {
    try {
      ...
      const authToken = localStorage.getItem("authToken"); // optional if backend uses JWT

      if (!userEmail) return;

      const res = await fetch(`${API_BASE}/api/myOrderData`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(authToken && { "auth-token": authToken }), // attach token only if present
        },
                  ...
                  {(order.items || []).map((item, index) => (
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <p className="font-bold mt-3 text-right text-green-400">
                          ₹{item.price ?? 0}/-
                        </p>
                      </div>
                    </div>
